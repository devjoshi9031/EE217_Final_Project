...
void readarray(float *image_part1,int *rowcol){
    ...
    rowcol[0]=dibheader.height;
    rowcol[1]=dibheader.width;


//    int  I_sz=dibheader.height*dibheader.width;
//    image_part= (unsigned char*) malloc( sizeof(unsigned char)*I_sz );
...
//            printf("%d\n ", image_part[i][j]);
            if ((float)image_part[i][j]>=0.0 && image_part[i][j]<=255.0){
            image_part1[i*dibheader.width+j]=(float)image_part[i][j];}
            else{image_part1[i*dibheader.width+j]=0.0;exit(1);}
        }
        printf("\n");
    }
