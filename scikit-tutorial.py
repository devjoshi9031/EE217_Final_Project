...
for file in range(0,10):
    ...
    cpu_error_rate = (cpu_error/127)*100
    ...
    gpu_time2= time.time()
    # calculating the gpu error and error rate
    gpu_error = np.sqrt(np.mean(np.square(ret-img.astype(np.float32))))
    gpu_error_rate = (gpu_error/127)*100
